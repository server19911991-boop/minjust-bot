...
print("🔄 Добавление вопросов по административному праву...")
...
admin_questions = load_questions('Тесты административное право.txt')

print(f"📝 Найдено {len(admin_questions)} вопросов по административному праву")
...
for q in admin_questions:
    ...
    if 'Блок Административное право' not in q.get('article', ''):
        q['article'] = q.get('article', '') + ' (Блок Административное право)'
    existing.append(q)
    added += 1
...
# Проверяем результат
admin_count = len([q for q in existing if 'Блок Административное право' in q.get('article', '')])
print(f"📊 Всего вопросов с пометкой 'Административное право': {admin_count}")

# Статистика по категориям
print(f"\n📊 Статистика по категориям:")
print(f"  - Вопросы по административному праву: {len(admin_questions)}")
