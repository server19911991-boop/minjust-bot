...
print("🔄 Добавление вопросов по банкротству...")
...
bankruptcy_questions = load_questions('Закон об урегулировании неплатежеспособности.txt')

print(f"📝 Найдено {len(bankruptcy_questions)} вопросов по банкротству")

# 5. Добавляем вопросы с новыми ID
added = 0

for q in bankruptcy_questions:
    max_id += 1
    q['id'] = max_id
    # Убеждаемся, что есть категория
    if 'Блок Банкротство' not in q.get('article', ''):
        q['article'] = q.get('article', '') + ' (Блок Банкротство)'
    existing.append(q)
    added += 1
...
# Проверяем результат
bankruptcy_count = len([q for q in existing if 'Блок Банкротство' in q.get('article', '')])
print(f"📊 Всего вопросов с пометкой 'Банкротство': {bankruptcy_count}")

# Статистика по категориям
print(f"\n📊 Статистика по категориям:")
print(f"  - Вопросы по банкротству: {len(bankruptcy_questions)}")
