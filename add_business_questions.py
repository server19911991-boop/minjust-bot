...
print("🔄 Добавление вопросов по хозяйственным обществам...")
...
business_questions = load_questions('Закон о хоз обществах.txt')

print(f"📝 Найдено {len(business_questions)} вопросов по хозяйственным обществам")
...
for q in business_questions:
    ...
    if 'Блок Закон о хозяйственных обществах' not in q.get('article', ''):
        q['article'] = q.get('article', '') + ' (Блок Закон о хозяйственных обществах)'
    existing.append(q)
    added += 1
...
# Проверяем результат
business_count = len([q for q in existing if 'Блок Закон о хозяйственных обществах' in q.get('article', '')])
print(f"📊 Всего вопросов с пометкой 'Закон о хозяйственных обществах': {business_count}")

# Статистика по категориям
print(f"\n📊 Статистика по категориям:")
print(f"  - Вопросы по хозяйственным обществам: {len(business_questions)}")
...
for q in existing:
    ...
    if 'Конституционное' in article and 'судопроизводство' not in article:
        ...
        cat = 'Банкротство'
    elif 'Закон о хозяйственных обществах' in article:
        cat = 'Закон о хозяйственных обществах'
    else:
        cat = 'Другие вопросы'
