...
print("🔄 Добавление вопросов по исполнительному производству...")
...
execution_questions = load_questions('тесты исполнительное производство.txt')

print(f"📝 Найдено {len(execution_questions)} вопросов по исполнительному производству")
...
for q in execution_questions:
    ...
    if 'Блок Исполнительное производство' not in q.get('article', ''):
        q['article'] = q.get('article', '') + ' (Блок Исполнительное производство)'
    existing.append(q)
    added += 1
...
# Проверяем результат
execution_count = len([q for q in existing if 'Блок Исполнительное производство' in q.get('article', '')])
print(f"📊 Всего вопросов с пометкой 'Исполнительное производство': {execution_count}")

# Статистика по категориям
print(f"\n📊 Статистика по категориям:")
print(f"  - Вопросы по исполнительному производству: {len(execution_questions)}")
